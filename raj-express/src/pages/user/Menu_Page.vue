  ...
  setup() {
    ...
    const fetchUserData = async () => {
      try {
        ...
        const response = await axios.get('http://localhost/AMBOT-KAPOYA-NA/raj-express/backend/controller/update-profile.php', {
          headers: {
    ...
    const deleteAccount = () => {
      $q.dialog({
        ...
        try {
          const token = localStorage.getItem('authToken');
          await axios.delete('http://localhost/AMBOT-KAPOYA-NA/raj-express/backend/controller/auth/delete_account.php', {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
